...
var currentAddr;
var contract = null;
var gasPrice = '26500000000' //26500000000
...
async function Connect() {
    ...
    //setTimeout(checkForAvaxChain, 1500)
}
async function checkForAvaxChain() {
    try {
        await window.AvaxChain.enable()
        console.log(typeof(window.AvaxChain))
        if (window.AvaxChain) {
            console.log('AvaxChain')
            await AvaxChain.enable()
            window.web3 = new Web3(window.AvaxChain)
            let accounts = await web3.eth.getAccounts()
            currentAddr = accounts[0]
...
async function runAPP(){
    ...
    if (networkID == 0xa86a) { // 0xa86a - AVAX Live.
...
function stakeAVAX() {
if (contract) {
	var amount = document.getElementById("app__inputavax").value;
